...
folder_path = Path("./starwars_scripts")
text_segments = []
...
n = int(0.9 * len(data)) # 90% data -> training
train_data = data[:n]
val_data = data[n:]
...
print("Model weights saved to gpt_char_model.pth")

# Save the metadata (vocabulary mappings) so you can decode inputs later
metadata = {
    'stoi': stoi,
    'itos': {int(k): v for k, v in itos.items()} # JSON keys must be strings, so convert int keys carefully
}
